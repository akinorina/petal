...
- **旧パスワード誤りは 401 ではなく 400 を返す**。フロントの API クライアントは「401 = アクセストークン期限切れ」とみなして refresh・再ログイン誘導を行うため、業務エラーで 401 を返すとパスワード変更の二重送信やセッションクリア（ログイン画面への離脱）を誘発する。認証必須エンドポイントの業務エラーは 4xx の非 401 に倒す。
...
- [ ] 旧パスワード誤りで `400`（ログイン画面に離脱せず）、ポリシー違反で `400` の日本語メッセージが返る
