...
- アップロード成功時は再読込し、新規画像が含まれる最終ページへ自動遷移する。
...
- ページ位置補正: アップロード成功時に **最後のページ**（新規画像が含まれる）へ自動遷移するため、`onUploadSuccess` ハンドラで `setCurrentPage(Math.ceil(total/pageSize))` 相当の処理を実行。
...
8. 13 件目をアップロード → 完了後に 2 ページ目（最終ページ）へ自動遷移し、新規画像が含まれている。
