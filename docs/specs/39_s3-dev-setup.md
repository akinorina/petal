...
- `petal-images-dev` バケットの CORS 設定
- Lambda 環境変数 `S3_BUCKET` を `petal-images-dev` に更新（`deploy:dev` 再実行）
...
| 項目 | 状態 |
| ---- | ---- |
| `petal-images-dev` バケット（ap-northeast-1） | 作成済み |
| Lambda 実行ロールへの S3 権限付与 | 設定済み（`petal-images-dev/*` に PutObject / GetObject / DeleteObject / HeadObject） |
| `.envs/.env.dev` の `S3_BUCKET=petal-images-dev` | 設定済み |
...
バケット: `petal-images-dev`（ap-northeast-1）

### AWS コンソール手順

1. AWS コンソール → S3 → `petal-images-dev` → **アクセス許可** タブ
...
`.envs/.env.dev` はすでに `S3_BUCKET=petal-images-dev` になっているため、
...
| 変数名 | 開発環境値 | 説明 |
| ------ | ---------- | ---- |
| `S3_BUCKET` | `petal-images-dev` | S3 バケット名 |
| `S3_ENDPOINT` | （空） | Localstack 用エンドポイント。本番 S3 は空でよい |
| `S3_FORCE_PATH_STYLE` | `false` | Localstack 利用時のみ `true` |

## 7. 完了条件

- `petal-images-dev` バケットに CORS が設定されている
- Lambda 環境変数 `S3_BUCKET` が `petal-images-dev` になっている
