...
本番環境・Supabase 移行・AWS デプロイに必要な環境変数を `.env.example` として整備し、
...
- `backend/.env.example` の更新（Supabase 対応 DB 変数追加・コメント整備）
...
| 変数名 | 用途 |
| --- | --- |
| `DATABASE_URL` | Supabase PgBouncer Pooler 接続（port 6543）。通常の API 実行時に使用 |
| `DATABASE_URL_DIRECT` | Supabase Direct 接続（port 5432）。マイグレーション実行専用 |
