...
ファイル・ディレクトリを削除したいときは、`rm` で物理削除せず **リポジトリ直下の `.trash/` ディレクトリへ移動**する。

- `.trash/` は `.gitignore` 済みで Git 管理外。中身は人間が任意のタイミングで手動完全削除してよい。
- **AI エージェントも `rm` を使わず `mv <対象> .trash/` で退避する**。誤削除の保険になり、後から中身を確認・復元できる。
- 図の中間生成物（`.drawio` など）や一時ファイルの片付けも、この方法で行う。
- 同名ファイルの衝突を避けたい場合は退避時にリネーム（例: `mv foo.txt .trash/foo.$(date +%s).txt`）してよい。
