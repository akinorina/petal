...
- **セルフサービスのサインアップ**: **有効**（TSK-12 / [56_self-service-signup.md](56_self-service-signup.md)）。
  - 一般ユーザーは `POST /auth/signup`（Cognito `SignUp`）で自己登録できる。ロールは常に `user`（`admin` は管理者作成のみ）。
  - 管理者作成（`AdminCreateUser`）の招待フローも引き続き利用する（両者は併存）。
- **属性の検証 / アカウント確認**: **Email を検証**する設定にする。
  - セルフサインアップ時は Cognito が新規ユーザーへ**確認コード**をメール送信し、`POST /auth/confirm-signup`（`ConfirmSignUp`）でコードを確定する。
  - 管理者作成（招待メール）経由のユーザーは従来どおり `email_verified=true` で作成する。
