...
- S3（アプリ用バケット `petal-dev`）: `PutObject` / `GetObject` / `DeleteObject` / `HeadObject`
...
> S3 バケット（`petal-dev`）と Cognito User Pool、Neon のデータは `serverless remove` の対象外。必要に応じて手動で削除する。
